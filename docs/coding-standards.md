...
- **Before adding a new third-party package, check whether an already-mandatory dependency covers the same job, and check what the new package pulls in transitively.** `SixLabors.ImageSharp` was added standalone for Excel embedded-image decode/format-sniffing/PNG-encode — a job `SkiaSharp` (already mandatory for Chart/Gauge/PDF text) already covered, so it was pure redundant surface area. Removing it also surfaced that `PdfSharpCore` itself transitively depends on an old, CVE-flagged `SixLabors.ImageSharp` build — dropping the direct reference would have silently downgraded the resolved version instead of eliminating the package, since NuGet still needs *some* version to satisfy `PdfSharpCore`. Run `dotnet list package --include-transitive` before adding a package, not just before removing one. See `docs/decisions.md`'s "replace SixLabors.ImageSharp with SkiaSharp" entry (2026-07-28).
