...
**Also done (2026-07-26): simple (single-style) PDF text boxes now render real text on non-Windows**, without needing any part of the RichText/Uniscribe pipeline below. `Renderer.ProcessSimpleTextBox` branches on `!OperatingSystem.IsWindows()` to call a new `WriterBase.DrawWrappedText`/`PDFWriter` implementation that resolves the requested font family to one of PDF's standard 14 fonts (Helvetica/Times/Courier, via the existing `m_internalFonts` map), word-wraps using an approximate character-width table, and emits `BT/Tf/Td/Tj/ET` content-stream operators directly — no `System.Drawing`, Win32, SkiaSharp, or HarfBuzzSharp dependency. Verified end-to-end (temporarily forcing the branch on this Windows dev box, rendering a real PDF with correct word-wrap/position/font). Known gaps: multi-run "rich text" boxes (`ProcessRichTextBox`) are unchanged and still require the Windows-only pipeline; font family beyond the base-14 map falls back to Helvetica; no true Unicode beyond WinAnsi; left-alignment only. Full detail: `tasks/pdf-text-shaping-abstraction.md`'s "MVP" section, `docs/decisions.md`'s 2026-07-26 base-14 decision entry.
