...
| Chart rendering (3D) | Yes | Yes | `Draw3DSurface`/`Draw3DPolygon` virtual-dispatch chain converted to `IGraphicsPath` (Milestone D3-real); reaches the same production path as 2D now that Milestone F wires backend selection. `Pie3D`/`Doughnut3D` remain blocked by a separate, unrelated label-drawing gap (`PieChart.Draw3DInsideLabels`, tracked under Milestone B1b) |
