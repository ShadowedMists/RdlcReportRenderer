...
namespace Microsoft.Reporting.Chart.WebForms
{
	internal class ChartGraphics3D : ChartRenderingEngine
	{
		...
		/// <summary>
		/// Interface-typed counterpart of <see cref="Draw3DPolygon"/> (Milestone D3 — see
		/// chart-gdi-type-abstraction.md). Named with the <c>Resource</c> suffix rather than as an
		/// overload since none of this method's parameters are Pen/Brush/GraphicsPath-typed — there is
		/// nothing to overload on, same reason <c>FillRectangleRelResource</c>/<c>GetPolygonCirclePathResource</c>
		/// use the suffix elsewhere in this migration.
		/// </summary>
		internal IGraphicsPath Draw3DPolygonResource(ChartArea area, Matrix3D matrix, LightStyle lightStyle, SurfaceNames surfaceName, float positionZ, Color backColor, Color borderColor, int borderWidth, ChartDashStyle borderStyle, DataPoint3D firstPoint, DataPoint3D secondPoint, DataPoint3D thirdPoint, DataPoint3D fourthPoint, ArrayList points, int pointIndex, float tension, DrawingOperationTypes operationType, LineSegmentType lineSegmentType, SurfaceNames thinBorders)
		{
			IDrawingResourceFactory resourceFactory = ((ChartGraphics)this).ResourceFactory;
			bool flag = (operationType & DrawingOperationTypes.DrawElement) == DrawingOperationTypes.DrawElement;
			IGraphicsPath graphicsPath = ((operationType & DrawingOperationTypes.CalcElementPath) == DrawingOperationTypes.CalcElementPath) ? resourceFactory.CreatePath() : null;
			Point3D[] array = new Point3D[4]
			{
				new Point3D((float)firstPoint.xPosition, (float)firstPoint.yPosition, positionZ),
				new Point3D((float)secondPoint.xPosition, (float)secondPoint.yPosition, positionZ),
			...
			if (flag)
			{
				SmoothingMode smoothingMode = ((ChartGraphics)this).SmoothingMode;
				((ChartGraphics)this).SmoothingMode = SmoothingMode.Default;
				((ChartGraphics)this).FillPolygon(resourceFactory.CreateSolidBrush(polygonLight), array2);
				((ChartGraphics)this).SmoothingMode = smoothingMode;
				if (thinBorders != 0)
				{
					IPen pen2 = resourceFactory.CreatePen(color, 1f);
					if ((thinBorders & SurfaceNames.Left) != 0)
					{
						((ChartGraphics)this).DrawLine(pen2, array2[3], array2[0]);
					}
				...
				{
					((ChartGraphics)this).DrawPolygon(resourceFactory.CreatePen(polygonLight, 1f), array2);
				}
				pen = resourceFactory.CreatePen(color, borderWidth);
				pen.StartCap = LineCap.Round;
				pen.EndCap = LineCap.Round;
				((ChartGraphics)this).DrawLine(pen, array2[0], array2[1]);
				((ChartGraphics)this).DrawLine(pen, array2[2], array2[3]);
		...
		/// <summary>Interface-typed counterpart of <see cref="GetSplineFlattenPath"/> (Milestone D3 — see chart-gdi-type-abstraction.md).</summary>
		internal IGraphicsPath GetSplineFlattenPathResource(ChartArea area, Matrix3D matrix, float positionZ, float depth, DataPoint3D firstPoint, DataPoint3D secondPoint, ArrayList points, int pointIndex, float tension, bool flatten, bool translateCoordinates, int yValueIndex)
		{
			int num = (firstPoint.index < secondPoint.index) ? firstPoint.index : secondPoint.index;
			num--;
			if (num >= points.Count - 2)
		...
		/// <summary>Interface-typed counterpart of <see cref="Draw3DSplineSurface"/> (Milestone D3 — see chart-gdi-type-abstraction.md).</summary>
		internal IGraphicsPath Draw3DSplineSurfaceResource(ChartArea area, Matrix3D matrix, LightStyle lightStyle, SurfaceNames surfaceName, float positionZ, float depth, Color backColor, Color borderColor, int borderWidth, ChartDashStyle borderStyle, DataPoint3D firstPoint, DataPoint3D secondPoint, ArrayList points, int pointIndex, float tension, DrawingOperationTypes operationType, bool forceThinBorder, bool forceThickBorder, bool reversedSeriesOrder, bool multiSeries, int yValueIndex, bool clipInsideArea)
		{
			if (tension == 0f)
			{
				return Draw3DSurfaceResource(area, matrix, lightStyle, surfaceName, positionZ, depth, backColor, borderColor, borderWidth, borderStyle, firstPoint, secondPoint, points, pointIndex, tension, operationType, LineSegmentType.Single, forceThinBorder, forceThickBorder, reversedSeriesOrder, multiSeries, yValueIndex, clipInsideArea);
			}
			IDrawingResourceFactory resourceFactory = ((ChartGraphics)this).ResourceFactory;
			IGraphicsPath graphicsPath = ((operationType & DrawingOperationTypes.CalcElementPath) == DrawingOperationTypes.CalcElementPath) ? resourceFactory.CreatePath() : null;
			IGraphicsPath splineFlattenPath = GetSplineFlattenPathResource(area, matrix, positionZ, depth, firstPoint, secondPoint, points, pointIndex, tension, flatten: true, translateCoordinates: false, yValueIndex);
			bool flag = false;
			if (pointIndex + 1 < points.Count && ((DataPoint3D)points[pointIndex + 1]).index == firstPoint.index)
			{
				flag = true;
			...
			for (int i = 1; i < pathPoints.Length; i++)
			{
				...
				IGraphicsPath graphicsPath2 = Draw3DSurfaceResource(area, matrix, lightStyle, surfaceName, positionZ, depth, backColor, borderColor, borderWidth, borderStyle, dataPoint3D, dataPoint3D2, points, pointIndex, 0f, operationType, lineSegmentType, forceThinBorder2, forceThickBorder2, reversedSeriesOrder, multiSeries, yValueIndex, clipInsideArea);
				if (graphicsPath != null && graphicsPath2 != null && graphicsPath2.PointCount > 0)
				{
					graphicsPath.AddPath(graphicsPath2, connect: true);
				}
		...
		/// <summary>Interface-typed counterpart of <see cref="Draw3DSurface"/> (Milestone D3 — see chart-gdi-type-abstraction.md).</summary>
		internal IGraphicsPath Draw3DSurfaceResource(ChartArea area, Matrix3D matrix, LightStyle lightStyle, SurfaceNames surfaceName, float positionZ, float depth, Color backColor, Color borderColor, int borderWidth, ChartDashStyle borderStyle, DataPoint3D firstPoint, DataPoint3D secondPoint, ArrayList points, int pointIndex, float tension, DrawingOperationTypes operationType, LineSegmentType lineSegmentType, bool forceThinBorder, bool forceThickBorder, bool reversedSeriesOrder, bool multiSeries, int yValueIndex, bool clipInsideArea)
		{
			if (tension != 0f)
			{
				return Draw3DSplineSurfaceResource(area, matrix, lightStyle, surfaceName, positionZ, depth, backColor, borderColor, borderWidth, borderStyle, firstPoint, secondPoint, points, pointIndex, tension, operationType, forceThinBorder, forceThickBorder, reversedSeriesOrder, multiSeries, yValueIndex, clipInsideArea);
			}
			IDrawingResourceFactory resourceFactory = ((ChartGraphics)this).ResourceFactory;
			bool flag = (operationType & DrawingOperationTypes.DrawElement) == DrawingOperationTypes.DrawElement;
			IGraphicsPath graphicsPath = ((operationType & DrawingOperationTypes.CalcElementPath) == DrawingOperationTypes.CalcElementPath) ? resourceFactory.CreatePath() : null;
			...
			if (clipInsideArea)
			{
				...
				if ((decimal)firstPoint.yPosition < num2 || (decimal)firstPoint.yPosition > num4 || (decimal)secondPoint.yPosition < num2 || (decimal)secondPoint.yPosition > num4)
				{
					...
					if (flag2)
					{
						graphicsPath = Draw3DSurfaceResource(area, matrix, lightStyle, surfaceName, positionZ, depth, gradientColor, gradientColor2, borderWidth, borderStyle, firstPoint, secondPoint, points, pointIndex, tension, operationType, lineSegmentType, forceThinBorder, forceThickBorder, reversedSeriesOrder, multiSeries, yValueIndex, clipInsideArea);
						firstPoint.yPosition = yPosition;
						secondPoint.yPosition = yPosition2;
						return graphicsPath;
					}
			...
			IPen pen = resourceFactory.CreatePen(color, 1f);
			if (flag)
			{
				if (backColor != Color.Transparent)
				{
				...
				if (forceThinBorder || forceThickBorder)
				{
					if (forceThickBorder)
					{
						IPen pen2 = resourceFactory.CreatePen(color, borderWidth);
						pen2.StartCap = LineCap.Round;
						pen2.EndCap = LineCap.Round;
						((ChartGraphics)this).DrawLine(pen2, array2[0], array2[1]);
						((ChartGraphics)this).DrawLine(pen2, array2[2], array2[3]);
			...
			if (borderWidth > 1 && !forceThickBorder)
			{
				pen3 = resourceFactory.CreatePen(color, borderWidth);
				pen3.StartCap = LineCap.Round;
				pen3.EndCap = LineCap.Round;
				bool reversed = false;
				if (firstPoint.index > secondPoint.index)
			...
			if (area.Area3DStyle.Perspective == 0)
			{
				...
				if (flag)
				{
					frontLinePen = ((borderWidth > 1) ? pen3 : pen).Clone();
					frontLinePoint1 = array2[0];
					frontLinePoint2 = array2[1];
				}
			}
		...
		/// <summary>Interface-typed counterpart of the thin 18-arg <see cref="Fill3DRectangle(RectangleF, float, float, Matrix3D, LightStyle, Color, ChartHatchStyle, string, ChartImageWrapMode, Color, ChartImageAlign, GradientType, Color, Color, int, ChartDashStyle, PenAlignment, DrawingOperationTypes)"/> overload (Milestone D3 — see chart-gdi-type-abstraction.md).</summary>
		internal IGraphicsPath Fill3DRectangleResource(RectangleF position, float positionZ, float depth, Matrix3D matrix, LightStyle lightStyle, Color backColor, ChartHatchStyle backHatchStyle, string backImage, ChartImageWrapMode backImageMode, Color backImageTranspColor, ChartImageAlign backImageAlign, GradientType backGradientType, Color backGradientEndColor, Color borderColor, int borderWidth, ChartDashStyle borderStyle, PenAlignment penAlignment, DrawingOperationTypes operationType)
		{
			return Fill3DRectangleResource(position, positionZ, depth, matrix, lightStyle, backColor, 0f, 0f, backHatchStyle, backImage, backImageMode, backImageTranspColor, backImageAlign, backGradientType, backGradientEndColor, borderColor, borderWidth, borderStyle, penAlignment, BarDrawingStyle.Default, veticalOrientation: false, operationType);
		}

		/// <summary>Interface-typed counterpart of the main 22-arg <see cref="Fill3DRectangle(RectangleF, float, float, Matrix3D, LightStyle, Color, float, float, ChartHatchStyle, string, ChartImageWrapMode, Color, ChartImageAlign, GradientType, Color, Color, int, ChartDashStyle, PenAlignment, BarDrawingStyle, bool, DrawingOperationTypes)"/> overload (Milestone D3 — see chart-gdi-type-abstraction.md).</summary>
		internal IGraphicsPath Fill3DRectangleResource(RectangleF position, float positionZ, float depth, Matrix3D matrix, LightStyle lightStyle, Color backColor, float topRightDarkening, float bottomLeftDarkening, ChartHatchStyle backHatchStyle, string backImage, ChartImageWrapMode backImageMode, Color backImageTranspColor, ChartImageAlign backImageAlign, GradientType backGradientType, Color backGradientEndColor, Color borderColor, int borderWidth, ChartDashStyle borderStyle, PenAlignment penAlignment, BarDrawingStyle barDrawingStyle, bool veticalOrientation, DrawingOperationTypes operationType)
		{
			if (barDrawingStyle == BarDrawingStyle.Cylinder && base.ActiveRenderingType != RenderingType.Svg)
			{
				return Fill3DRectangleAsCylinderResource(position, positionZ, depth, matrix, lightStyle, backColor, topRightDarkening, bottomLeftDarkening, backHatchStyle, backImage, backImageMode, backImageTranspColor, backImageAlign, backGradientType, backGradientEndColor, borderColor, borderWidth, borderStyle, penAlignment, veticalOrientation, operationType);
			}
			IDrawingResourceFactory resourceFactory = ((ChartGraphics)this).ResourceFactory;
			Point3D[] array = new Point3D[8];
			IGraphicsPath graphicsPath = ((operationType & DrawingOperationTypes.CalcElementPath) == DrawingOperationTypes.CalcElementPath) ? resourceFactory.CreatePath() : null;
					...
					if (((double)depth != 0.0 || surfaceNames == SurfaceNames.Front) && ((double)position.Width != 0.0 || surfaceNames == SurfaceNames.Left || surfaceNames == SurfaceNames.Right) && ((double)position.Height != 0.0 || surfaceNames == SurfaceNames.Top || surfaceNames == SurfaceNames.Bottom))
					{
						...
						if ((flag && i == 1) || (!flag && i == 0))
						{
							...
							if ((operationType & DrawingOperationTypes.DrawElement) == DrawingOperationTypes.DrawElement)
							{
								if ((visibleSurfacesWithPerspective & surfaceNames) != 0)
								{
									((ChartGraphics)this).FillPolygon(resourceFactory.CreateSolidBrush(color), array2);
									if (surfaceNames == SurfaceNames.Front && barDrawingStyle != 0 && barDrawingStyle != BarDrawingStyle.Cylinder)
									{
										DrawBarStyleGradients(matrix, barDrawingStyle, position, positionZ, depth, veticalOrientation);
									}
								...
								if (lightStyle != 0 && (borderWidth == 0 || borderStyle == ChartDashStyle.NotSet || borderColor == Color.Empty))
								{
									pen = resourceFactory.CreatePen(color, 1f);
									pen.Alignment = PenAlignment.Inset;
								}
								pen.StartCap = LineCap.Round;
								pen.EndCap = LineCap.Round;
		...
		/// <summary>
		/// Retyped in place for Milestone D3 (see chart-gdi-type-abstraction.md) — private helper,
		/// no signature change, so no interface-typed sibling is needed (same reasoning as
		/// <c>ChartGraphics.DrawPointLabelBackground</c>).
		/// </summary>
		private void DrawBarStyleGradients(Matrix3D matrix, BarDrawingStyle barDrawingStyle, RectangleF position, float positionZ, float depth, bool isVertical)
		{
			...
			case BarDrawingStyle.Wedge:
			{
				...
				using (IGraphicsPath graphicsPath3 = resourceFactory.CreatePath())
				{
					...
					using (ISolidBrush brush3 = resourceFactory.CreateSolidBrush(Color.FromArgb(90, Color.Black)))
					{
				...
				using (IGraphicsPath graphicsPath4 = resourceFactory.CreatePath())
				{
					...
					using (ISolidBrush brush4 = resourceFactory.CreateSolidBrush(Color.FromArgb(50, Color.Black)))
					{
				...
				using (IGraphicsPath graphicsPath5 = resourceFactory.CreatePath())
				{
					...
					using (ISolidBrush brush5 = resourceFactory.CreateSolidBrush(Color.FromArgb(50, Color.Black)))
					{
			...
			case BarDrawingStyle.LightToDark:
			{
				...
				using (IGraphicsPath graphicsPath6 = resourceFactory.CreatePath())
				{
					...
					{
						using (ILinearGradientBrush brush6 = resourceFactory.CreateLinearGradientBrush(bounds, (!isVertical) ? Color.Transparent : Color.FromArgb(120, Color.White), (!isVertical) ? Color.FromArgb(120, Color.White) : Color.Transparent, isVertical ? 90f : 0f))
						{
				...
				using (IGraphicsPath graphicsPath7 = resourceFactory.CreatePath())
				{
					...
					{
						using (ILinearGradientBrush brush7 = resourceFactory.CreateLinearGradientBrush(bounds2, isVertical ? Color.Transparent : Color.FromArgb(80, Color.Black), isVertical ? Color.FromArgb(80, Color.Black) : Color.Transparent, isVertical ? 90f : 0f))
						{
			...
			case BarDrawingStyle.Emboss:
			{
				...
				{
					graphicsPath.AddPolygon(array2);
					using (ISolidBrush brush = resourceFactory.CreateSolidBrush(Color.FromArgb(100, Color.White)))
					{
				...
				using (IGraphicsPath graphicsPath2 = resourceFactory.CreatePath())
				{
					...
					using (ISolidBrush brush2 = resourceFactory.CreateSolidBrush(Color.FromArgb(80, Color.Black)))
					{
		...
		/// <summary>Interface-typed counterpart of <see cref="DrawMarker3D"/> (Milestone D3 — see chart-gdi-type-abstraction.md).</summary>
		internal IGraphicsPath DrawMarker3DResource(Matrix3D matrix, LightStyle lightStyle, float positionZ, PointF point, MarkerStyle markerStyle, int markerSize, Color markerColor, Color markerBorderColor, int markerBorderSize, string markerImage, Color markerImageTranspColor, int shadowSize, Color shadowColor, RectangleF imageScaleRect, DrawingOperationTypes operationType)
		{
			ChartGraphics chartGraphics = (ChartGraphics)this;
			IDrawingResourceFactory resourceFactory = chartGraphics.ResourceFactory;
			IGraphicsPath graphicsPath = ((operationType & DrawingOperationTypes.CalcElementPath) == DrawingOperationTypes.CalcElementPath) ? resourceFactory.CreatePath() : null;
			Point3D[] array = new Point3D[1]
			{
				new Point3D(point.X, point.Y, positionZ)
			...
			if (markerStyle != 0 && markerSize > 0 && markerColor != Color.Empty)
			{
				...
				case MarkerStyle.Circle:
					if ((operationType & DrawingOperationTypes.DrawElement) == DrawingOperationTypes.DrawElement)
					{
						if (shadowSize != 0 && shadowColor != Color.Empty)
						{
							if (!chartGraphics.softShadows)
							{
								ISolidBrush brush = resourceFactory.CreateSolidBrush((shadowColor.A != byte.MaxValue) ? shadowColor : Color.FromArgb((int)markerColor.A / 2, shadowColor));
								RectangleF rect = empty2;
								rect.X += shadowSize;
								rect.Y += shadowSize;
								chartGraphics.FillEllipse(brush, rect);
						...
						rect2.Inflate(rect2.Width / 4f, rect2.Height / 4f);
						graphicsPath3.AddEllipse(rect2);
						IPathGradientBrush pathGradientBrush2 = resourceFactory.CreatePathGradientBrush(graphicsPath3);
						pathGradientBrush2.CenterColor = ChartGraphics.GetGradientColor(markerColor, Color.White, 0.85);
						pathGradientBrush2.SurroundColors = new Color[1]
						{
							markerColor
						...
						pathGradientBrush2.CenterPoint = array4[0].PointF;
						chartGraphics.FillEllipse(pathGradientBrush2, empty2);
						chartGraphics.DrawEllipse(resourceFactory.CreatePen(markerBorderColor, markerBorderSize), empty2);
					}
					if ((operationType & DrawingOperationTypes.CalcElementPath) == DrawingOperationTypes.CalcElementPath)
					{
						graphicsPath.AddEllipse(empty2);
				...
				case MarkerStyle.Square:
				{
					...
					empty3.Width = relativeSize.Width;
					empty3.Height = relativeSize.Height;
					graphicsPath = Fill3DRectangleResource(empty3, positionZ - relativeSize.Width / 2f, relativeSize.Width, matrix, lightStyle, markerColor, ChartHatchStyle.None, "", ChartImageWrapMode.Scaled, Color.Empty, ChartImageAlign.Center, GradientType.None, Color.Empty, markerBorderColor, markerBorderSize, ChartDashStyle.Solid, PenAlignment.Outset, operationType);
					break;
				}
				default:
					throw new InvalidOperationException(SR.ExceptionGraphics3DMarkerStyleUnknown);
		...
		/// <summary>Interface-typed counterpart of <see cref="Fill3DRectangleAsCylinder"/> (Milestone D3 — see chart-gdi-type-abstraction.md).</summary>
		internal IGraphicsPath Fill3DRectangleAsCylinderResource(RectangleF position, float positionZ, float depth, Matrix3D matrix, LightStyle lightStyle, Color backColor, float topRightDarkening, float bottomLeftDarkening, ChartHatchStyle backHatchStyle, string backImage, ChartImageWrapMode backImageMode, Color backImageTranspColor, ChartImageAlign backImageAlign, GradientType backGradientType, Color backGradientEndColor, Color borderColor, int borderWidth, ChartDashStyle borderStyle, PenAlignment penAlignment, bool veticalOrientation, DrawingOperationTypes operationType)
		{
			IDrawingResourceFactory resourceFactory = ((ChartGraphics)this).ResourceFactory;
			Point3D[] array = new Point3D[8];
			IGraphicsPath graphicsPath = ((operationType & DrawingOperationTypes.CalcElementPath) == DrawingOperationTypes.CalcElementPath) ? resourceFactory.CreatePath() : null;
			...
			for (int j = 0; j <= 1; j++)
			{
				...
				for (int num2 = 1; num2 <= 32; num2 *= 2)
				{
					...
					if ((flag && j == 1) || (!flag && j == 0))
					{
						IGraphicsPath graphicsPath4 = null;
						_ = new PointF[4];
						Color color = backColor;
						IBrush brush = null;
						switch (surfaceNames2)
						...
						{
							color = backColor;
							graphicsPath4 = resourceFactory.CreatePath();
							PointF leftSideLinePoint = PointF.Empty;
							PointF rightSideLinePoint = PointF.Empty;
							AddEllipseSegment(graphicsPath4, graphicsPath3, graphicsPath2, matrix.perspective == 0f && veticalOrientation, num, out leftSideLinePoint, out rightSideLinePoint);
							graphicsPath4.Reverse();
							...
							if (!leftSideLinePoint.IsEmpty && !rightSideLinePoint.IsEmpty && !leftSideLinePoint2.IsEmpty && !rightSideLinePoint2.IsEmpty)
							{
								...
								if (linesIntersection.X != linesIntersection2.X || linesIntersection.Y != linesIntersection2.Y)
								{
									ILinearGradientBrush linearGradientBrush = resourceFactory.CreateLinearGradientBrush(linesIntersection, linesIntersection2, gradientColor, gradientColor2);
									ColorBlend colorBlend = new ColorBlend(5);
									colorBlend.Colors[0] = gradientColor2;
									colorBlend.Colors[1] = gradientColor2;
									colorBlend.Colors[2] = gradientColor;
						...
						if (graphicsPath4 != null)
						{
							if ((operationType & DrawingOperationTypes.DrawElement) == DrawingOperationTypes.DrawElement)
							{
								if ((surfaceNames & surfaceNames2) != 0)
								{
									((ChartGraphics)this).FillPath((brush == null) ? resourceFactory.CreateSolidBrush(color) : brush, graphicsPath4);
								}
								IPen pen = resourceFactory.CreatePen(borderColor, borderWidth);
								pen.DashStyle = ((ChartGraphics)this).GetPenStyle(borderStyle);
								if (lightStyle != 0 && (borderWidth == 0 || borderStyle == ChartDashStyle.NotSet || borderColor == Color.Empty))
								{
									pen = resourceFactory.CreatePen((brush == null) ? color : ChartGraphics.GetGradientColor(backColor, Color.Black, 0.3), 1f);
									pen.Alignment = PenAlignment.Inset;
								}
								pen.StartCap = LineCap.Round;
								pen.EndCap = LineCap.Round;
		...
		/// <summary>
		/// Interface-typed overload of <see cref="AddEllipseSegment(GraphicsPath, GraphicsPath, GraphicsPath, bool, float, out PointF, out PointF)"/>
		/// (Milestone D3 — see chart-gdi-type-abstraction.md). Unlike the other <c>ChartGraphics3D</c>
		/// D3 conversions, this one is a true overload (not a <c>Resource</c>-suffixed sibling) since
		/// its 3 <see cref="GraphicsPath"/> parameters are themselves enough to distinguish it from the
		/// concrete original. The concrete original stays — <c>FunnelChart.DrawFunnelCircularSegment</c>'s
		/// 3D branch is a documented, permanent B2 holdout (see chart-gdi-type-abstraction.md) that still
		/// calls it with concrete <see cref="GraphicsPath"/> instances.
		/// </summary>
		internal void AddEllipseSegment(IGraphicsPath resultPath, IGraphicsPath ellipseFlattenPath, IGraphicsPath oppositeEllipseFlattenPath, bool veticalOrientation, float cylinderAngle, out PointF leftSideLinePoint, out PointF rightSideLinePoint)
		{
			leftSideLinePoint = PointF.Empty;
			rightSideLinePoint = PointF.Empty;
			if (ellipseFlattenPath.PointCount == 0)
