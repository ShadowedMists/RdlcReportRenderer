...
namespace Microsoft.Reporting.Gauge.WebForms
{
	internal class XamlRenderer : IDisposable
	{
		...
		public void ParseXaml(RectangleF viewportRect, Color[] layerHues, int? maxLayers = null)
		{
			...
			int num = maxLayers.HasValue ? maxLayers.Value : CountChildNodes(xmlNode);
		...
		private RectangleF GetRootCanvasBoundingRectangle(XmlNode rootCanvas)
		{
			...
			{
				throw new Exception(Utils.SRGetStr("ExceptionXamlInvalidCanvasSize"));
			}
		...
		{
			XamlLayer xamlLayer = new XamlLayer();
			...
			foreach (XmlNode childNode in canvasNode.ChildNodes)
			{
				...
				else
				{
					...
					{
						throw new Exception(Utils.SRGetStr("ExceptionXamlShapeNotSupported", childNode.Name));
					}
		...
		private IBrush CreateBrush(XmlNode brushNode, int layerIndex, RectangleF shapeRect, RectangleF originalShapeRect, RectangleF fromBounds, RectangleF toBounds)
		{
			...
			else
			{
				...
				{
					throw new Exception(Utils.SRGetStr("ExceptionXamlBrushNotSupported", brushNode.Name));
				}
				float num = 0.5f;
		...
		private RectangleF GetStreamGeometryBounds(string[] parts, bool includeOrigin)
		{
			...
			using (GraphicsPath graphicsPath = new GraphicsPath())
			{
				...
				for (int i = 0; i < parts.Length; i++)
				{
					...
					{
						throw new Exception(Utils.SRGetStr("ExceptionXamlGeometryNotSupported", parts[i]));
					}
				}
		...
		private void IntepretStreamGeometry(string[] parts, PointF shapeOffset, float stretchFactorX, float stretchFactorY, bool includeOrigin, RectangleF fromBounds, RectangleF toBounds, ref IGraphicsPath graphicsPath)
		{
			...
			throw new Exception(Utils.SRGetStr("ExceptionXamlGeometryNotSupported", parts[num]));
		}
		...
		{
			if (LayerHues == null || layerIndex >= LayerHues.Length || LayerHues[layerIndex] == Color.Transparent)
			{
				return color;
			}
