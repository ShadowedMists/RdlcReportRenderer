	...
	internal class XmlFormatSerializer : SerializerBase
	{
		...
		private void SerializeObject(object objectToSerialize, object parent, string elementName, XmlNode xmlParentNode, XmlDocument xmlDocument)
		{
			...
			if (base.TemplateMode && parent is IList)
			{
				...
				{
					xmlAttribute.Value = ((IList)parent).IndexOf(objectToSerialize).ToString(CultureInfo.CurrentCulture);
				}
			...
			{
				if ((flag && propertyInfo.Name == "Name") || IsMapBaseProperty(objectToSerialize, parent, propertyInfo))
				{
					continue;
				}
				...
				{
					if (!propertyInfo.CanRead || !propertyInfo.CanWrite || propertyInfo.Name == "Item" || propertyInfo.Name == "Tag")
					{
						continue;
					}
		...
		private string GetXmlValue(object obj, object parent, string elementName)
		{
			...
			if (obj is Image)
			{
				return ImageToString((Image)obj);
			}
			if (obj is ShapeData)
			{
				return ShapeData.ShapeDataToString((ShapeData)obj);
			}
			if (obj is Stream)
			{
				return SerializerBase.StreamToString((Stream)obj);
			}
			if (obj is DateTime)
			{
			...
			if (obj is Margins)
			{
				return ((Margins)obj).ToString();
			}
			if (obj is MapCoordinate)
			{
				return ((MapCoordinate)obj).ToString();
			}
			if (obj is Type)
			{
		...
		public override void Deserialize(object objectToDeserialize, object reader)
		{
			...
			{
				throw new ArgumentException("Invalid type of reader object. Can be Stream, TextReader, XmlReader or String (file name)", "reader");
			}
			XmlDocument xmlDocument = new XmlDocument();
			if (reader is Stream)
			{
		...
		protected virtual int DeserializeObject(object objectToDeserialize, object parent, string elementName, XmlNode xmlParentNode, XmlDocument xmlDocument)
		{
			...
			foreach (XmlNode childNode2 in xmlParentNode.ChildNodes)
			{
				...
				else
				{
					...
					{
						throw new InvalidOperationException("Can't deserialize property. Unknown property name \"" + childNode2.Name + "\" in object " + objectToDeserialize.GetType().ToString());
					}
				}
			}
			return num;
		...
		private void SetXmlValue(object obj, string attrName, string attrValue)
		{
			...
			if (property != null)
			{
				...
				else if (property.PropertyType == typeof(ShapeData))
				{
					value = ShapeData.ShapeDataFromString(attrValue);
				}
				else if (property.PropertyType == typeof(Stream))
				{
					value = SerializerBase.StreamFromString(attrValue);
				}
				else
				{
			...
			{
				throw new InvalidOperationException("Can't deserialize property. Unknown property name \"" + attrName + "\" in object " + obj.GetType().ToString());
			}
		}
	}
}
