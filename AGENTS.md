...
- **A virtual method's overrides all share one signature — trace the whole override family before concluding a return-type change is blocked.** When one override in the family has a genuine concrete-type entanglement (e.g. `AreaChart`'s `ClipTopPoints`/`ClipBottomPoints`) and the others don't, the fix is to convert the entangled override's own dependencies first, not to declare the whole family permanently blocked — the entanglement is usually narrower than "the whole virtual slot."
