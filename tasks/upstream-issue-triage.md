...
- ~~**#241 "System.Security.Cryptography.Xml high security vulnerabilities"**~~ — checked 2026-07-31: `dotnet list package --vulnerable --include-transitive` across every project in the solution reports zero vulnerable packages (direct or transitive) against current NuGet advisory sources. This fork's `System.Security.Cryptography.Xml` `10.0.10` reference is not flagged. No action needed; re-run this check if dependencies are bumped or advisory sources change.
