...
- **`ProcessRichTextBox` (multi-run/mixed-style text) is not covered** - still calls the Windows-only `LineBreaker`/`TextBox` path unconditionally, regardless of platform. A textbox with per-run bold/italic/color changes within one paragraph will not render its text at all on non-Windows yet.
...
- **Word-wrap widths are approximate**, not exact glyph metrics - cosmetic imprecision only (see above), not a correctness risk, but real Adobe AFM-exact wrapping would look better.
- **Center/right text alignment is not implemented** - `DrawWrappedText` is left-aligned only; this was a deliberate scope cut (an unverified per-line-position scheme was judged riskier than shipping left-align-only and documenting the gap, per AGENTS.md's "document a genuine gap honestly rather than risk a subtly wrong port").
- **No underline/strikethrough, no vertical/rotated text, no highlight runs** on this path - `RPLTextBoxProps`' styling beyond family/size/bold/italic/color is not read yet.
- **No automated test exercises the cross-platform branch itself** - see `SimpleTextboxRdlTests`'s doc comment: `Renderer`/`PDFWriter` are internal with no `InternalsVisibleTo` grant to any test project (strong-name signed assembly), and the OS-check has no injectable seam the way Chart's constructor-based Skia/Gdi factory does. Adding one would mean either weakening the platform check in production code or touching signing infrastructure - both judged out of scope for unsupervised action. The manual verification above is the current evidence; a future increment could add a proper seam (e.g. an internal factory + a signed test project) if this needs to be a standing regression gate.
