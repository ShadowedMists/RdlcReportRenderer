...
| D2 — backend selection by platform | **In progress** | ~41 of ~49 real-blocker call sites converted (`ImageLoader.GetAdjustedImageSize` + DPI/CompositingQuality/InterpolationMode abstraction). Remaining ~8-site tail is the `Paint(Graphics,...)` chain itself — see Open items. |
...
*Remaining ~8-site tail (not yet converted — this is the real D2 payoff, since it's the only place a raw `Graphics` gets constructed/assigned rather than just read):*
- **`ChartPicture.Paint(Graphics graph, bool)`** (2-arg convenience overload, forwards to the real 8-arg `Paint(Graphics, bool, RenderingType, XmlTextWriter, Stream, string, bool, bool)`) — the single raw-`Graphics` entry point everything else funnels into. 4 real callers, all already holding an abstracted `IRenderSurface` and downcasting to `GdiRenderSurface` purely to reach `.NativeGraphics` before calling `Paint`: `ChartImage.GetImage`, `ChartImage.SaveImage`, `ChartImage.GetSvgImage`, `ChartPicture.Select`. Converting `Paint`'s signature to accept the render-surface abstraction directly (instead of `Graphics`) would eliminate all 4 downcasts in one pass — this is the actual "backend selection has something to select between" moment, and the natural next increment.
- **`ChartPicture.Select`**'s `chartGraph.Graphics = gdiRenderSurface.NativeGraphics` assignment — same `Paint`-chain conversion covers this.
- **`SelectionManager.Annotation.DrawSelection(Graphics g)`** (`Utilities/SelectionManager.cs`) — reassigns `graph.Graphics = g` redundantly (it's already set from the active `Paint` call); resolved by the same conversion, not a separate fix.
- **`TextAnnotation.GetContentPosition`**'s standalone `new Bitmap(...)` + `Graphics.FromImage(...)` construction (for measuring annotation content size before any chart-attached `ChartGraphics` exists) — a genuine second GDI+-only construction site independent of the `Paint` chain. Needs its own fix (route through `IRenderSurfaceFactory` like `GetSvgImage` does) before it stops being Windows-only; not part of the `Paint` conversion above.
- **`FunnelChart.DrawFunnelCircularSegment`** (3D branch) and **`ChartImage.SaveIntoMetafile`** (EMF/HDC construction) remain the two already-documented permanent holdouts (B2 holdouts / D1 notes) — deliberately out of scope, not part of this tail.
