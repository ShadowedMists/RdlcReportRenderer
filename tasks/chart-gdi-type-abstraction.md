...
- [ ] **B2. Convert `ChartGraphics` high-level methods** (`DrawLineRel`, `FillRectangleAbs`, …) to construct resources via the factory. Depends on B1b. 🟠 L
  **Scope re-measured, materially larger than L:** `ChartGraphics.GetGradientBrush`/`GetHatchBrush`/`GetTextureBrush` (the exact three methods B1b's `solidBrush` blocker depends on) are called from **20+ files under `Microsoft.Reporting.Chart.WebForms.ChartTypes\`** (one per chart type — `AreaChart.cs`, `BarChart.cs`, etc. — the "Painters (73 files)" of the design diagram in §1). Retyping their return type to `IBrush` is a real, valuable step, but ripples into every one of those chart-type files, each of which independently mixes `Brush`/`Font`/`StringFormat`/`GraphicsPath` in the same methods (the same bundling problem noted in the Milestone C strategic finding). **New, bigger discovery: `GaugeGraphics` (`Microsoft.Reporting.Gauge.WebForms`) and `MapGraphics` (`Microsoft.Reporting.Map.WebForms`) each have their own separate, parallel `GetHatchBrush`/`GetGradientBrush` methods** returning concrete `Brush` — i.e. there are **three** GDI+-coupled rendering engines (chart, gauge, map), not two. The parent plan (`chart-cross-platform-implementation.md` Phase 5) already calls out "repeat for the gauge engine" but did not mention Map at all — Map's cross-platform status needs its own decision (is map rendering in scope for this initiative, or does it stay Windows-only?). Recommend: before starting B2 for real, do a short scoping pass over `ChartTypes/*.cs` (which of the 20+ files are genuinely independent vs. GraphicsPath-bundled, mirroring the C1-C4 approach) and get an explicit answer on Map's scope, rather than converting `ChartGraphics`'s brush-helper methods blind and discovering the ripple mid-edit.
