...
**Status: DONE (2026-07-27), short of the explicitly-out-of-scope categories below.** Documented 2026-07-26, immediately after the obsolete-API/CA2200/CS0169/CS0649 cleanup pass (see git history same day); all 11 categories (NU1510, CS1717, CS0472, CS0809, CS0108, CS0109, CS0162, CS0414, the 3 "likely real bug" CS0649 fields, CS0219, CA2022) fixed 2026-07-26/27. This doc covers what's left in a full `dotnet build ReportViewerCore.sln -t:Rebuild` that is realistically fixable without a large architectural project — as opposed to the two already-tracked deep migrations (`tasks/webrequest-httpclient-migration.md`, `tasks/xmlvalidatingreader-migration.md`) and the CA1416 platform-support warnings, which are the existing Chart/Gauge/PDF/Map cross-platform migration itself (see `TODO.md`, `docs/rendering-abstractions.md`) — not a warning-cleanup task, just this project's normal work restated as warnings. The remaining ~54 CS0649 fields (section 9 below) are explicitly still open — each needs its own judgment call, not a mechanical pass.
...
## 10. CS0219 — assigned-but-unused local variables (166 warnings, 59 files) — DONE (2026-07-27)

Fixed via 4 parallel delegated agents (one per file-group, disjoint file sets to avoid conflicts), each instructed with the same A/B/C triage this doc specified. Outcome: all 83 unique sites resolved, 82 were bucket A (dead null/zero/false initializer, immediately shadowed by an early return or unconditional overwrite — deleted with zero behavior change), 1 was bucket C and correctly left untouched:

- `GaugeCore.cs`'s `CheckLicense()` builds a registry key path and a `.lic` filename into locals, then returns a hardcoded `false` without consulting either — looks like real license-check logic that was stripped down when this fork removed Dundas Gauge licensing, not a dropped check worth restoring. Flagged, not fixed.

Verified: `dotnet build --no-incremental` 0 errors; full suite `Chart.Rdl.Tests` 106/106, `ReportViewerCore.LinuxRenderers.Tests` 15/15, `VisualRegressionTests` 137/137 (pixel-exact — many Chart/Gauge/Map files were touched), 0 regressions.

---

## 11. CA2022 — inexact `Stream.Read` (74 warnings, 18 files) — DONE (2026-07-27)

Fixed via 3 parallel delegated agents, each instructed to verify the actual stream source at every site (in-memory vs. genuinely streamed) before swapping, per this doc's own caution. Outcome: **every one of the 37 unique sites turned out to read from an already-fully-buffered source** (a `MemoryStream` explicitly constructed a few lines earlier, an embedded manifest resource, or a stream independently confirmed elsewhere in the codebase to always be a `MemoryStream`) — including the two sites this doc specifically flagged as worth extra scrutiny:
- `PDFWriter.cs:314` (`DrawDynamicImage`) — traced `RPLDynamicImageProps.DynamicImageContent` to two other call sites (`HTML4Renderer.cs`, `HTML5Renderer.cs`) that explicitly cast it `as MemoryStream`, confirming it's never a genuinely streamed source.
- `AsyncMainStreamRenderingOperation.cs:30` — despite the "Async...Streaming" name, the stream is a locally-created `MemoryStream` used as a rendering sink, fully written and rewound before this read.

None of the existing code had a loop or partial-read handling around any of these calls, so every swap to `Stream.ReadExactly(byte[], int, int)` closes a latent correctness bug (silent truncation → `EndOfStreamException`), not just the warning.

Verified: `dotnet build --no-incremental` 0 errors, 0 CA2022 remaining; same full-suite verification as CS0219 above (106/106 + 15/15 + 137/137, 0 regressions).
