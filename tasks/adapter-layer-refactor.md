...
**Re-audited 2026-07-28: every bullet below is now resolved or already documented — most were stale cross-references to work completed elsewhere, not open work.** Kept here as a record of what was checked, not an active list.
...
- Investigate support options for spatial SQL types such as SqlGeography and document the current limitation. *(done — already documented in README.md's "Known permanent limitations"; verified 2026-07-28 that this bullet was just a stale cross-reference. `DataAggregate.DataTypeCode.SqlGeography`/`SqlGeometry` are just enum type-tags the RDL engine still recognizes; `ExprHostBuilder` still emits `Imports Microsoft.SqlServer.Types` into every generated expression-host source file, but since no `Microsoft.SqlServer.Types`-compatible package is referenced, VB.NET downgrades the unresolvable import to a warning (not an error) rather than failing compilation for every report — confirmed empirically, ordinary non-spatial reports compile and render fine throughout this session's testing. Only a report whose expressions actually reference the real spatial type/methods would fail, matching README's "reports depending on them won't load" claim exactly. No further investigation needed unless someone wants actual spatial-type support, which would mean sourcing a cross-platform-compatible spatial types package — a real feature addition, not a bug fix.)*
...
- Assess whether interactive web preview can be supported on ASP.NET Core without a major rewrite, or document it as out of scope. *(done — already documented in README.md's "Known permanent limitations" as a permanent gap, with HTML5/HTML4.0 rendering named as the substitute; verified 2026-07-28 this was another stale cross-reference, not open work.)*
- Investigate single-file deployment compatibility for Roslyn and runtime assembly loading. *(done — see `tasks/expression-compiler-modernization.md`, fixed and verified end-to-end 2026-07-27 via a real published-single-file harness.)*
- Validate the current status of the map control and capture any known limitations. *(done — `MapRdlTests.cs` (added 2026-07-27) confirms the existing Windows-only behavior actually works; see `tasks/test-coverage-gaps.md`. Migration itself remains deferred per `docs/decisions.md`, which is a separate, already-tracked concern, not this bullet's scope.)*
