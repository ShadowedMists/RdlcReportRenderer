...
**Chart engine** (`tasks/chart-gdi-type-abstraction.md`): Milestones A (foundation), B (chokepoint migration), C1-C8 (per-type ports + call-site conversion) are done. Open: D2 (backend selection — blocked one level below where scoped, on `ChartImage.GetImage`'s hard GDI+-typed return contract), D3 (remove temporary concrete overloads — blocked on `ChartGraphics3D.cs`, `MapAreasCollection.cs`, a few named holdouts, and the entire Gauge engine), E1/E2 (Skia backend beyond the validating spike).
