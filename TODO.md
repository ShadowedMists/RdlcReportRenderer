...
Infrastructure complete. Excel Phases 4-5 complete. Chart engine GDI+→interface abstraction is substantially complete (permanently blocked on one item by design) — **but a 2026-07-28 discovery found Chart/Gauge model objects couldn't even be constructed on Linux at all until a same-day fix; the Font gap and the follow-on HotRegionsList/GraphicsPath gap are both now fixed, but Linux rendering still hits a third GDI+ wall (`Label.Paint`'s raw `StringFormat` construction) — see `tasks/chart-default-font-cross-platform.md` and `tasks/chart-hotregion-graphicspath-cross-platform.md`.** Gauge engine GDI+→interface abstraction is complete (no Skia backend yet — not started, low priority). PDF cross-platform rendering is complete. Map engine migration is deferred. Full detail on all of these lives in `docs/`, not here — see the Documentation index below.
...
| 🔴 **HIGH** | Chart: `Label.Paint` (and likely others) constructs raw `System.Drawing.StringFormat` directly | 📋 NOT STARTED — newly discovered 2026-07-28 as the next blocker after the GraphicsPath/HotRegionsList gap above cleared; same class of GDI+ wall (StringFormat can't construct on Linux either); `ITextFormat`/`CreateTextFormat()` already exist as the portable equivalent, already used elsewhere in the Font-gap fixes; see `tasks/chart-default-font-cross-platform.md`'s "New gap found: Label.Paint's raw StringFormat construction" section | HIGH |
