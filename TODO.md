...
**Chart engine** (`tasks/chart-gdi-type-abstraction.md`): Milestones A (foundation), B (chokepoint migration), C1-C8 (per-type ports + call-site conversion) are done. D2 (backend selection) is **in progress** (2026-07-22): scoped its real blocker to ~49 call sites reading `.Graphics` as raw `System.Drawing.Graphics`, converted ~41 of them (`ImageLoader.GetAdjustedImageSize`'s dead `Graphics` param removed; DPI/`CompositingQuality`/`InterpolationMode` abstracted onto `IChartRenderingEngine`). Remaining ~8-site tail is the `ChartPicture.Paint(Graphics,...)` entry-point chain itself (`ChartImage.GetImage`/`SaveImage`/`GetSvgImage`, `ChartPicture.Select`) plus one independent `TextAnnotation.cs` GDI+-only construction site — see task doc for exact detail. D3 (remove temporary concrete overloads — blocked on `ChartGraphics3D.cs`, `MapAreasCollection.cs`, a few named holdouts, and the entire Gauge engine), E1/E2 (Skia backend beyond the validating spike) still not started.
