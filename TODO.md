...
| 🟡 **HIGH** | Chart engine: GDI+ → interface abstraction | 🔄 Substantially complete (Milestones A-C done; D2/D3/E1/E2 open) | HIGH |
...
**Chart engine** (`tasks/chart-gdi-type-abstraction.md`): Milestones A (foundation), B (chokepoint migration), C1-C8 (per-type ports + call-site conversion) are done. D2 (backend selection) is **in progress** (2026-07-22): scoped its real blocker to ~49 call sites reading `.Graphics` as raw `System.Drawing.Graphics`, and converted ~45 of them across two sub-increments — (1) `ImageLoader.GetAdjustedImageSize`'s dead `Graphics` param removed, DPI/`CompositingQuality`/`InterpolationMode` abstracted onto `IChartRenderingEngine`; (2) `ChartPicture.Paint`'s `Graphics`-typed entry point given an `IRenderSurface`-typed dual-overload sibling (via a new `IChartRenderingEngine.BindSurface(IRenderSurface)` that moves the Gdi-surface downcast into the backend adapters), all 4 real production callers (`ChartImage.GetImage`/`SaveImage`/`GetSvgImage`, `ChartPicture.Select`) converted; the `Graphics`-typed overload is kept permanently for `ChartImage.SaveIntoMetafile`'s EMF-only path. Remaining ~4-site tail (a `TextAnnotation.cs` GDI+-only construction site, a harmless redundant reassignment in `SelectionManager.cs`, and two already-documented permanent holdouts) — see task doc for exact detail. D3 (remove temporary concrete overloads — blocked on `ChartGraphics3D.cs`, `MapAreasCollection.cs`, a few named holdouts, and the entire Gauge engine), E1/E2 (Skia backend beyond the validating spike) still not started.
