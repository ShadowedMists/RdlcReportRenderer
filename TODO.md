...
**`Widen` half of the `Widen`/`Region` blocker resolved (2026-07-19):** implemented `SkiaGraphicsPath.Widen(IPen)` for real via `SKPaint.GetFillPath` (Skia's own stroke-to-fill primitive, not hand-rolled), added the missing `ChartGraphics.Widen(IGraphicsPath, IPen)` overload and 2 more missing `HotRegionsList.AddHotRegion` interface-typed overloads, then converted 9 of the 13 real `ChartGraphics.Widen` call sites across `LineChart.cs`, `StepLineChart.cs`, `KagiChart.cs`, `RadarChart.cs`, `LineAnnotation.cs`, `PolylineAnnotation.cs`, `Axis.cs`, and the hit-region-only halves of `AreaChart.cs`/`StackedAreaChart.cs`. **`Region`-from-path turned out bigger than scoped** — the remaining blocked files don't just need `new Region(path)` (already solvable) but read/reassign `ChartRenderingEngine.Clip`, a `public Region` property on the whole engine's base class; retyping it is Milestone-D-adjacent work, not a per-file fix, and is now tracked as its own separate item. Full detail, including the 4 sites left blocked on unrelated deeper issues, in `chart-gdi-type-abstraction.md`.
