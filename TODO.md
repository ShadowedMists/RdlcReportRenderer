...
- [~] Phase 5: Apply same pattern to gauge engine — **started** (`tasks/gauge-gdi-type-abstraction.md`, Milestone A equivalent done, see above) — **and decide Map.WebForms's scope** (a third GDI+-coupled engine, not yet started)
- [ ] Phase 6: Visual regression testing (extend the E0 harness — `tests/Microsoft.ReportViewer.DataVisualization.VisualRegressionTests/` — with a real Skia render path and a broader report corpus; the Gauge engine needs its own equivalent harness built from scratch first)
