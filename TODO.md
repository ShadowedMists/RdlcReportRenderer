...
| 🔴 **HIGH** | Chart/Gauge: model classes' default Font crashed Linux at *construction* time (not just rendering) | ✅ Font gap fully fixed for Chart (2026-07-28) — construction-time crash, Legend's and Axis's auto-fit measurement/drawing paths all ported to IChartFont and WSL-verified (crash now moves completely past all Font construction into an unrelated `GraphicsPath`/`HotRegionsList` gap, tracked separately); see `tasks/chart-default-font-cross-platform.md` | HIGH |
| 🔴 **HIGH** | Chart: `HotRegionsList.AddHotRegion` constructs raw `System.Drawing.Drawing2D.GraphicsPath` for tooltip hit-testing | 📋 NOT STARTED — newly discovered 2026-07-28 as the next blocker after the Font gap above cleared; same class of GDI+ wall as Font (GraphicsPath can't construct on Linux either); `IGraphicsPath` already exists as a portable equivalent elsewhere in Chart's migrated code; see `tasks/chart-default-font-cross-platform.md`'s "New gap found" section | HIGH |
