...
**Gauge B1/B2 begun (2026-07-21), user-directed ("Begin B1 and B2").** B1 turned out already satisfied: `GaugeGraphics : RenderingEngine` inherits `RenderingEngine.ResourceFactory` (added during A3), so no new field was needed — unlike Chart, where `ChartGraphics` needed its own field since it doesn't inherit from a factory-carrying base. For B2, traced actual callers before converting anything (same discipline as Chart's B2 investigation) and found `GaugeGraphics.cs`'s design-time/interactive selection-drawing cluster (`GetDesignTimeSelectionFillBrush`, `GetDesignTimeSelectionBorderPen`, `DrawSelection`, the marker half of `DrawRadialSelection`) is genuinely self-contained — no shared multi-branch brush locals, no concrete `GraphicsPath` coupling — and converted it fully to `IBrush`/`IPen`, adding a new `GetSelectionPenResource` sibling where a caller (`DrawRadialSelection`'s path-drawing call) still needs a concrete `Pen`. Found and fixed one small real gap along the way: `IPen` (shared interface) had no `DashPattern` member, needed for the selection outline's custom dash cadence — added it to `IPen` and its 3 implementers (Gauge `GdiPen`, Chart `GdiPen`, Chart `SkiaPen` spike stub). **The much larger, actually-exercised brush cluster** (`GetHatchBrush`/`GetGradientBrush`/`GetPieGradientBrush`/`GetTextureBrush` and their shared-local consumers across `BackFrame.cs`/`StateIndicator.cs`/etc.) hit the identical shared-local blocker Chart's B2 already found and documented in detail — deliberately not attempted this pass to avoid repeating Chart's reverted attempts; left as the next real chunk of work. Verified: build 0 errors, full suite still 54/54, zero baseline diffs (expected — the converted cluster isn't exercised by either current sample). Full findings in `tasks/gauge-gdi-type-abstraction.md` Milestone B.

**Gauge B2 groundwork continued (2026-07-21), user-directed ("Proceed", following a review of Chart's B2 findings).** Added interface-typed siblings (`GetHatchBrushResource`, `GetGradientBrushResource`, `GetPieGradientBrushResource`) for the three brush-getters in `GaugeGraphics.cs` that are genuinely self-contained — no shared field, no image-loading dependency — mirroring Chart's own C4 "build the port before any caller can migrate" step. The real callers (`BackFrame.GetBrush`, `DrawPathAbs`, etc.) still can't migrate: they interchange these three getters' results with `GetTextureBrush`'s in a shared `Brush` local, and `GetTextureBrush` needs `common.ImageLoader`'s image-loading path bridged to an interface type first — the identical shape of Chart's own multi-session image-loading/DPI saga, deliberately not attempted speculatively. Also found (not fixed) a new small gap: `GetMarkerBrush`'s gradient branches use `LinearGradientBrush.Transform`/`PathGradientBrush.RotateTransform`/`TranslateTransform`, which have no equivalent on `ILinearGradientBrush`/`IPathGradientBrush` yet. Verified: build 0 errors, full suite 54/54, zero baseline diffs. Full findings in `tasks/gauge-gdi-type-abstraction.md` Milestone B.
...
- [~] Phase 5: Apply same pattern to gauge engine — **started** (`tasks/gauge-gdi-type-abstraction.md`, Milestone A + visual-regression harness + B1/B2 first slice done, see above) — **and decide Map.WebForms's scope** (a third GDI+-coupled engine, not yet started)
